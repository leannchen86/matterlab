...
function MoveBayModal({ checks, setChecks, scanned, setScanned, feedback, appendLog, onFinish, onClose }: { checks: Record<string, boolean>; setChecks: React.Dispatch<React.SetStateAction<Record<string, boolean>>>; scanned: boolean; setScanned: (value: boolean) => void; feedback: string; appendLog: (type: string, text: string, add?: number) => void; onFinish: (correct: boolean) => void; onClose: () => void }) {
  const items = [['jack', 'Jack condition + rated load'], ['restraint', 'Tote restraint + pallet condition'], ['route', 'Aisle, door + receiving bay clear']];
...
function FacilityCompleteModal({ scores, logCount, exceptionCount, onDeck, onClose }: { scores: Scores; logCount: number; exceptionCount: number; onDeck: () => void; onClose: () => void }) {
  ...
  return <ModalShell title="Shift debrief" kicker="WO-3024 · COMPLETE" onClose={onClose}><div className="debrief-score"><span>SHIFT RATING</span><b>{total}</b><i>/ 100</i></div><p className="modal-intro">You controlled the physical move, preserved tote identity, proved the gas-service boundary, and bounded the result window before AI ingestion.</p><DebriefVisual scenario="facility" scores={scores} exceptionCount={exceptionCount} /><div className="debrief-grid"><span>Safety<b>{scores.safety}</b></span><span>Traceability<b>{scores.traceability}</b></span><span>Data integrity<b>{scores.integrity}</b></span><span>Uptime<b>{scores.uptime}</b></span></div><div className="lesson-card"><b>Technician leverage</b><p>Material identity and utility state are scientific context. A high-quality result is not comparable evidence until the physical boundary and measurement-control window are known.</p></div><p className="debrief-meta">{logCount} shift events captured · move, service, control, and eligibility boundaries linked</p><button className="modal-run" type="button" onClick={onDeck}>CHOOSE ANOTHER SHIFT</button></ModalShell>;
}
