...
export function FacilityShift({ onSwitch }: { onSwitch: (id: ScenarioId) => void }) {
  ...
  const finishControl = (correct: boolean) => {
    ...
    setPhase(4); setSelectedId('BET-02'); reward({ uptime: scores.uptime + 7, integrity: scores.integrity + 6 }); appendLog('qc', 'ALU-21 reference passed at 181 m²/g, inside the 173–191 m²/g range. The full test record was saved.', 4); setFeedback(''); setModal('evidence');
  };
...
function getFacilityAction(phase: number, actions: (() => void)[]) {
  return [
    ...
    { tag: 'NEXT STEP', title: 'The load is ready to move', metric: '184 kg', body: 'The correct container is secured and the aisle is clear.', label: 'MOVE THE MATERIAL', fn: actions[1], tone: 'ready' },
...
function GasBoundaryModal({ checks, setChecks, leakRan, setLeakRan, feedback, appendLog, onFinish, onClose }: { checks: Record<string, boolean>; setChecks: React.Dispatch<React.SetStateAction<Record<string, boolean>>>; leakRan: boolean; setLeakRan: (value: boolean) => void; feedback: string; appendLog: (type: string, text: string, add?: number) => void; onFinish: (correct: boolean) => void; onClose: () => void }) {
  ...
  return <ModalShell title="Check the new gas connection" kicker="STEP 3 · GAS CHECK" onClose={onClose} wide><div className="facility-workbench gas-workbench"><div className="facility-visual-panel"><div className="panel-heading"><span>MANIFOLD MFD-2 · ISOLATED TEST</span><b>{leakRan ? 'LEAK CHECK PASSED' : 'ANALYZER PAUSED'}</b></div><GasManifoldVisual checks={checks} leakRan={leakRan} /><div className="gas-readout"><span>SUPPLY<b>{checks.trend ? '2.08 bar' : '—'}</b></span><span>DECAY<b>{leakRan ? '0.7 µbar·L/s' : '—'}</b></span><span>LIMIT<b>&lt; 2.0</b></span></div></div><div className="facility-control-panel"><p className="modal-intro">Match the gas label, isolate the analyzer, and check for leaks. Cylinder color alone is not enough.</p><div className="evidence-brief">{items.map(([key, label]) => <article key={key}><i>•</i><div><b>{label}</b><small>Checked during the isolated leak test.</small></div></article>)}</div><button className="modal-run" type="button" disabled={leakRan} onClick={runLeak}>{leakRan ? 'GAS LEAK CHECK PASSED' : 'RUN GAS LEAK CHECK'}</button>{leakRan && <div className="decision-stack"><button type="button" onClick={() => onFinish(true)}>Save the passing gas check</button><button type="button" className="secondary" onClick={() => onFinish(false)}>Accept from cylinder color and pressure</button></div>}</div></div>{feedback && <p className="feedback bad">{feedback}</p>}<p className="concept-note">Simulation only · follow local gas and isolation procedures in a real lab.</p></ModalShell>;
}
...
function FacilityEvidenceModal({ feedback, onFinish, onClose }: { feedback: string; onFinish: (correct: boolean) => void; onClose: () => void }) {
  return <ModalShell title="Separate results across the gas change" kicker="STEP 5 · RESULT CHECK" onClose={onClose} wide><div className="decision-question"><span>DECISION</span><b>Which results were produced after the new gas connection was proven?</b></div><div className="transition-evidence"><div className="transition-chart"><div className="panel-heading"><span>RESULTS OVER TIME</span><b>GAS CHANGE + REFERENCE CHECK</b></div><ServiceTransitionChart /><div className="transition-legend"><span><i className="pre" />BEFORE PROOF</span><span><i className="control" />REFERENCE</span><span><i className="post" />AFTER PROOF</span></div></div><aside className="transition-window"><span>RESULT GROUPS</span><div className="window-row held"><i>01</i><b>RUNS ADS-401 / 402</b><em>UNCERTAIN · HOLD</em></div><div className="window-row control"><i>02</i><b>ALU-21 · 181 m²/g</b><em>REFERENCE · PASS</em></div><div className="window-row ready"><i>03</i><b>RUNS ADS-403 / 404</b><em>AFTER CHECK · READY</em></div><p>The passing reference proves the state after the gas change. It cannot rewrite earlier results.</p></aside></div><div className="ai-proposal"><div><span>AUTOMATED SUGGESTION · SIMULATED</span><h3>Merge all six results into one batch</h3><p>The suggestion assumes every run is comparable across the gas change.</p></div><b>WAITING</b></div><div className="decision-stack horizontal evidence-actions"><button type="button" onClick={() => onFinish(true)}>Hold uncertain runs and use only results after the check</button><button type="button" className="secondary" onClick={() => onFinish(false)}>Merge every result together</button></div>{feedback && <p className="feedback bad">{feedback}</p>}</ModalShell>;
}
...
function FacilityCompleteModal({ scores, logCount, exceptionCount, onDeck, onClose }: { scores: Scores; logCount: number; exceptionCount: number; onDeck: () => void; onClose: () => void }) {
  ...
  return <ModalShell title="Mission debrief" kicker="WO-3024 · COMPLETE" onClose={onClose}><div className="debrief-score"><span>RUN RATING</span><b>{total}</b><i>/ 100</i></div><p className="modal-intro">You moved the correct container, proved the gas connection, tested the analyzer, and separated uncertain results.</p><DebriefVisual scenario="facility" scores={scores} exceptionCount={exceptionCount} /><div className="debrief-grid"><span>Safety<b>{scores.safety}</b></span><span>Traceability<b>{scores.traceability}</b></span><span>Data integrity<b>{scores.integrity}</b></span><span>Uptime<b>{scores.uptime}</b></span></div><p className="score-explanation"><b>Score note:</b> Lower category scores reflect blocked choices and recovery time. {exceptionCount} blocked attempt{exceptionCount === 1 ? "" : "s"}.</p><div className="lesson-card"><b>Key lesson</b><p>A good result is only comparable when the physical material, gas connection, and machine-check window are known.</p></div><p className="debrief-meta">{logCount} run events captured · move, service, control, and result boundaries linked</p><button className="modal-run" type="button" onClick={onDeck}>CHOOSE ANOTHER MISSION</button></ModalShell>;
}
