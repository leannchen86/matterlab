...
const tasks = ['Choose the correct container', 'Move the material', 'Check the new gas', 'Test the analyzer', 'Review results across the gas change'];
...
export function FacilityShift({ onSwitch }: { onSwitch: (id: ScenarioId) => void }) {
  ...
  const stations = useMemo(() => baseStations.map((station): Station => {
    if (station.id === 'PREP-01') {
      const bay = { ...station, name: 'Material staging bay', purpose: 'Identifies incoming containers and secures them for a controlled move.' };
      if (phase === 0) return { ...bay, state: 'MOVE HOLD', tone: 'warn', meta: 'Two containers · identity unresolved', technicianView: ['Requested container: A', 'Staging bay: 2 containers', 'Pallet jack: inspection due'] };
      if (phase === 1) return { ...bay, state: 'TRANSFER READY', tone: 'ready', meta: 'Container A · secured', technicianView: ['Container: A', 'Restraint: checked', 'Route: clear'] };
      return { ...bay, state: 'BAY RELEASED', tone: 'ready', meta: 'Transfer receipt linked', technicianView: ['Move: MOV-3024', 'Departure scan: linked', 'Spill check: clear', 'Staging bay: available'] };
    }
  ...
  return <main className="shell scenario-shell scenario-facility" style={{ '--scenario-accent': '#68d4ad' } as React.CSSProperties}>
    ...
    <div className="workspace"><aside className="left-rail"><section className="rail-section shift-card"><p className="section-kicker">CURRENT MISSION</p><h2>Move material and verify a gas change</h2></section><section className="rail-section"><p className="section-kicker">MISSION STEPS</p><ol className="task-list">{tasks.map((title, index) => { const done = phase > index; const active = !done && index === phase; return <Task key={title} number={`0${index + 1}`} title={title} status={done ? 'done' : active ? 'active' : 'pending'} onClick={active ? actions[index] : undefined} />; })}</ol></section></aside>
      <section className="lab-view"><LabViewport stations={stations} selectedId={selectedId} phase={phase} scenarioId="facility" inspectionState={physicalInspections} onInspectionChange={recordInspection} onSelect={setSelectedId} /></section>
      <aside className="right-rail"><section className={`rail-section alert-card tone-${action.tone}`}><p>{action.body}</p><button className="primary-action" type="button" onClick={action.fn}>{action.label}<span>→</span></button></section><section className="rail-section station-inspector"><div className="station-identity"><b>{selected.id}</b><h2 title={selected.purpose}>{selected.name}</h2></div><StationAccess station={selected} scenarioId="facility" physicalChecks={physicalInspections[selected.id] ?? []} /></section></aside></div>
...
function getFacilityAction(phase: number, actions: (() => void)[]) {
  return [
    { tag: 'NEXT STEP', title: 'Choose the correct container', body: 'Two containers look alike. Match the label and secure the load before moving it.', label: 'CHECK THE MOVE', fn: actions[0], tone: 'warn' },
    { tag: 'NEXT STEP', title: 'The load is ready to move', body: 'The correct container is secured and the aisle is clear.', label: 'MOVE MATERIAL', fn: actions[1], tone: 'ready' },
    { tag: 'NEXT STEP', title: 'Check the new gas connection', body: 'Confirm the gas label, isolation, and leak test before using the analyzer.', label: 'CHECK THE GAS', fn: actions[2], tone: 'warn' },
    { tag: 'NEXT STEP', title: 'Test the analyzer after the change', body: 'Run the training QC material to prove the new setup works.', label: 'RUN THE CHECK', fn: actions[3], tone: 'ready' },
    { tag: 'NEXT STEP', title: 'Which results can be reused?', body: 'A passing check only proves the analyzer worked after the gas change.', label: 'REVIEW THE RESULTS', fn: actions[4], tone: 'warn' },
    { tag: 'MISSION COMPLETE', title: 'Move and gas change complete', body: 'The material moved safely and only trustworthy results will be reused.', label: 'VIEW SUMMARY', fn: actions[5], tone: 'ready' },
  ][phase] ?? { tag: 'MISSION COMPLETE', title: 'Move and gas change complete', body: 'The controlled move is complete.', label: 'VIEW SUMMARY', fn: actions[5], tone: 'ready' };
}
...
function MoveBayVisual({ scanned, checks }: { scanned: boolean; checks: Record<string, boolean> }) {
  return <svg className="move-bay-svg" viewBox="0 0 620 360" role="img" aria-label="Powered pallet jack, two staged powder totes, and controlled aisle route">
    ...
    <text x="110" y="44" textAnchor="middle">NORTH POSITION</text><text x="437" y="44" textAnchor="middle">SOUTH POSITION</text><text x="540" y="330" textAnchor="end">RECEIVING →</text><text x="80" y="330">AISLE A-2 · {checks.route ? 'CLEAR' : 'VERIFY ROUTE'}</text>
  </svg>;
}
...
function BetControlModal({ setChecks, acquired, onAcquire, clearFeedback, feedback, onFinish, onClose }: { checks: Record<string, boolean>; setChecks: React.Dispatch<React.SetStateAction<Record<string, boolean>>>; acquired: boolean; onAcquire: () => void; clearFeedback: () => void; feedback: string; onFinish: (correct: boolean) => void; onClose: () => void }) {
  ...
  return <ModalShell title="Test the analyzer" kicker="STEP 4 · MACHINE CHECK" onClose={onClose} wide><div className="facility-workbench bet-control-workbench"><div className="facility-visual-panel"><div className="panel-heading"><span>REFERENCE MATERIAL</span><b>{acquired ? 'CHECK PASSED' : 'READY TO TEST'}</b></div><BetControlVisual acquired={acquired} /><div className="gas-readout bet-control-readout"><span>RESULT<b>{acquired ? 'IN RANGE' : 'NOT RUN'}</b></span></div></div><div className="facility-control-panel"><button className="modal-run" type="button" disabled={acquired} onClick={acquire}>{acquired ? 'QC CHECK PASSED' : 'RUN QC CHECK'}</button>{!acquired && <button className="control-certificate-shortcut" type="button" onClick={() => onFinish(false)}>Reuse an old certificate value</button>}{acquired && <div className="decision-stack"><button type="button" onClick={() => onFinish(true)}>Save the passing check</button></div>}</div></div>{feedback && <p className="feedback bad">{feedback}</p>}</ModalShell>;
}
...
function FacilityEvidenceModal({ feedback, onFinish, onClose }: { feedback: string; onFinish: (correct: boolean) => void; onClose: () => void }) {
  return <ModalShell title="Separate results across the gas change" kicker="STEP 5 · RESULT CHECK" onClose={onClose} wide><div className="decision-question"><span>DECISION</span><b>Which results did the passing QC check actually cover?</b></div><div className="transition-evidence"><div className="transition-chart"><div className="panel-heading"><span>RESULT ORDER</span><b>GAS CHANGE + QC CHECK</b></div><ServiceTransitionChart /><div className="transition-legend"><span><i className="pre" />BEFORE CHECK</span><span><i className="control" />QC CHECK</span><span><i className="post" />AFTER CHECK</span></div></div></div><div className="decision-stack horizontal evidence-actions"><button type="button" onClick={() => onFinish(true)}>Use only results after the pass</button><button type="button" className="secondary" onClick={() => onFinish(false)}>Use everything because QC passed</button></div>{feedback && <p className="feedback bad">{feedback}</p>}</ModalShell>;
}
