...
export function TgaShift({ onSwitch }: { onSwitch: (id: ScenarioId) => void }) {
  ...
  const finishPan = (correct: boolean) => {
    ...
    setPhase(2); reward({ traceability: scores.traceability + 14, integrity: scores.integrity + 6 }); appendLog('lineage', 'Mixed pan pair set aside. Matching platinum PANSET-14 loaded for method THM-208.', 7); window.setTimeout(() => setModal(null), 650);
  };
...
function getTgaAction(phase: number, actions: (() => void)[]) {
  return [
    ...
    { tag: 'IN PROGRESS', title: 'The sample is heating', metric: '64%', body: 'Advance when you are ready to inspect the result.', label: 'SEE THE RESULT', fn: actions[3], tone: 'run' },
...
function BaselineModal({ setChecks, ran, setRan, feedback, appendLog, onFinish, onClose }: { checks: Record<string, boolean>; setChecks: React.Dispatch<React.SetStateAction<Record<string, boolean>>>; ran: boolean; setRan: (value: boolean) => void; feedback: string; appendLog: (type: string, text: string, add?: number) => void; onFinish: (correct: boolean) => void; onClose: () => void }) {
  ...
  return <ModalShell title="Review the failed empty-pan check" kicker="STEP 1 · MACHINE CHECK" onClose={onClose} wide><div className="modal-grid scenario-bench-grid"><div><p className="modal-intro">The machine drifted even with no sample loaded. Repeat the check so the bad reading is saved instead of hidden.</p><div className="evidence-brief">{items.map(([key, title, note]) => <article key={key}><i>•</i><div><b>{title}</b><small>{note}</small></div></article>)}</div><button className="modal-run" type="button" disabled={ran} onClick={run}>{ran ? 'FAILED CHECK SAVED' : 'REPEAT EMPTY-PAN CHECK · 8 MIN'}</button></div><div className="instrument-console"><div className="panel-heading"><span>MASS (mg) + HEAT FLOW (mW) VS TIME</span><b>PAN POSITIONS A / B</b></div><TgaTrace baseline ran={ran} /><div className="result-box"><span>MASS OFFSET</span><b>{ran ? '+0.28 mg' : '—'}</b><span>HEAT-FLOW SLOPE</span><b>{ran ? '+0.42 mW' : '—'}</b></div>{ran && <div className="decision-stack"><p className="mini-label">WHAT SHOULD HAPPEN NEXT?</p><button type="button" onClick={() => onFinish(true)}>Save the failure and inspect the pans</button><button type="button" className="secondary" onClick={() => onFinish(false)}>Hide the offset and continue</button></div>}</div></div>{feedback && <p className="feedback bad">{feedback}</p>}</ModalShell>;
}
...
function BlankControlModal({ setChecks, acquired, onAcquire, feedback, onFinish, onClose }: { checks: Record<string, boolean>; setChecks: React.Dispatch<React.SetStateAction<Record<string, boolean>>>; acquired: boolean; onAcquire: () => void; feedback: string; onFinish: (correct: boolean) => void; onClose: () => void }) {
  ...
  return <ModalShell title="Run the empty-pan test" kicker="STEP 3 · SETUP TEST" onClose={onClose} wide><div className="modal-grid scenario-bench-grid"><div><p className="modal-intro">The pans now match. Run one empty test to prove both measurement channels are stable.</p><div className="evidence-brief">{items.map(([key, title]) => <article key={key}><i>•</i><div><b>{title}</b><small>{key === 'pair' ? 'The physical pans and method agree.' : key === 'condition' ? 'Residue or damage would ruin the comparison.' : 'The gas flow and heating program are saved with the result.'}</small></div></article>)}</div><button className="modal-run" type="button" disabled={acquired} onClick={acquire}>{acquired ? 'EMPTY TEST SAVED' : 'RUN EMPTY-PAN TEST'}</button>{!acquired && <button className="blank-release-shortcut" type="button" onClick={() => onFinish(false)}>Skip the test because the pans match</button>}</div><div className="instrument-console"><div className="panel-heading"><span>MASS (mg) + HEAT FLOW (mW) VS TIME</span><b>THM-208 · REV 4</b></div><TgaBlankTrace acquired={acquired} /><div className="result-box"><span>MASS OFFSET</span><b>{acquired ? '+0.03 mg' : '—'}</b><span>HEAT-FLOW SLOPE</span><b>{acquired ? '+0.04 mW' : '—'}</b></div>{acquired && <div className="blank-verdict"><span>TEST RESULT</span><b>MASS + HEAT FLOW PASS</b><i>READY</i></div>}{acquired && <div className="decision-stack"><button type="button" onClick={() => onFinish(true)}>Save the test and release the sample</button></div>}</div></div>{feedback && <p className="feedback bad">{feedback}</p>}<p className="concept-note">Simulation only · real pan preparation and limits depend on the method and instrument.</p></ModalShell>;
}
...
function ThermalEvidenceModal({ feedback, onFinish, onClose }: { feedback: string; onFinish: (correct: boolean) => void; onClose: () => void }) {
  return <ModalShell title="Review the unusual thermal result" kicker="STEP 4 · RESULT CHECK" onClose={onClose} wide><div className="decision-question"><span>DECISION</span><b>Is the mass change caused by the sample, or could the gas flow have caused it?</b></div><div className="evidence-grid"><div className="trace-panel"><div className="panel-heading"><span>MASS + HEAT FLOW + GAS FLOW</span><b>TGA-01 · LOT-91-T</b></div><TgaTrace /><div className="axis"><span>25 °C</span><b>TEMPERATURE</b><span>800 °C</span></div></div><div className="report-panel"><div className="panel-heading"><span>EVENT TIMING</span><b>method rev 4</b></div><div className="report-metric"><span>Mass change</span><b>412.0 °C</b></div><div className="report-metric"><span>Gas-flow change</span><b>412.5 °C</b></div><div className="report-metric"><span>Heat-flow event</span><b>438 °C</b></div><div className="report-status warn-status">CAUSE NOT PROVEN</div><p>The mass and gas-flow changes happen together. That calls for a repeat, not a conclusion.</p></div></div><div className="ai-proposal"><div><span>AUTOMATED SUGGESTION · SIMULATED</span><h3>Lower the material’s heating temperature by 25 °C</h3><p>The suggestion treats the 412 °C change as sample behavior before a repeat.</p></div><b>WAITING</b></div><div className="decision-stack horizontal evidence-actions"><button type="button" onClick={() => onFinish(true)}>Repeat with matching pans before changing anything</button><button type="button" className="secondary" onClick={() => onFinish(false)}>Change the temperature now</button></div>{feedback && <p className="feedback bad">{feedback}</p>}</ModalShell>;
}
...
function TgaCompleteModal({ scores, logCount, exceptionCount, onDeck, onClose }: { scores: Scores; logCount: number; exceptionCount: number; onDeck: () => void; onClose: () => void }) {
  ...
  return <ModalShell title="Mission debrief" kicker="WO-2987 · COMPLETE" onClose={onClose}><div className="debrief-score"><span>RUN RATING</span><b>{total}</b><i>/ 100</i></div><p className="modal-intro">You saved the failed baseline, matched the pans, proved the empty setup, and refused to overclaim an uncertain result.</p><DebriefVisual scenario="tga" scores={scores} exceptionCount={exceptionCount} /><div className="debrief-grid"><span>Safety<b>{scores.safety}</b></span><span>Traceability<b>{scores.traceability}</b></span><span>Data integrity<b>{scores.integrity}</b></span><span>Uptime<b>{scores.uptime}</b></span></div><p className="score-explanation"><b>Score note:</b> Lower category scores reflect blocked choices and recovery time. {exceptionCount} blocked attempt{exceptionCount === 1 ? "" : "s"}.</p><div className="lesson-card"><b>Key lesson</b><p>A thermal curve is only trustworthy when the pan pair, gas flow, baseline, sample mass, and temperature stay linked.</p></div><p className="debrief-meta">{logCount} run events captured · baseline and native traces retained</p><button className="modal-run" type="button" onClick={onDeck}>CHOOSE ANOTHER MISSION</button></ModalShell>;
}
