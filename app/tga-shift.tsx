...
const tasks = [
  { title: 'Read the setup note', pending: 'Baseline check failed', done: 'Setup note read' },
...
function getTgaAction(phase: number, actions: (() => void)[]) {
  return [
    ...
    { tag: 'MISSION COMPLETE', title: 'The result is safely held for a repeat', metric: '5 / 5', body: 'You fixed the setup and did not overclaim an uncertain result.', label: 'VIEW SUMMARY', fn: actions[5], tone: 'ready' },
...
function BaselineModal({ setChecks, ran, setRan, feedback, appendLog, onFinish, onClose }: { checks: Record<string, boolean>; setChecks: React.Dispatch<React.SetStateAction<Record<string, boolean>>>; ran: boolean; setRan: (value: boolean) => void; feedback: string; appendLog: (type: string, text: string, add?: number) => void; onFinish: (correct: boolean) => void; onClose: () => void }) {
  ...
  return <ModalShell title="TGA/DSC baseline disposition" kicker="QC + HMI · TGA-01" onClose={onClose} wide><div className="modal-grid scenario-bench-grid"><div><p className="modal-intro">The empty-pan baseline is outside the release criterion. Review what the repeat controls, then run it without masking the excursion.</p><div className="evidence-brief">{items.map(([key, title, note]) => <article key={key}><i>•</i><div><b>{title}</b><small>{note}</small></div></article>)}</div><button className="modal-run" type="button" disabled={ran} onClick={run}>{ran ? 'BASELINE RETAINED' : 'REPEAT EMPTY-PAN BASELINE · 8 MIN'}</button></div><div className="instrument-console"><div className="panel-heading"><span>MASS (mg) + HEAT FLOW (mW) VS TIME</span><b>PAN POSITIONS A / B</b></div><TgaTrace baseline ran={ran} /><div className="result-box"><span>MASS OFFSET</span><b>{ran ? '+0.28 mg' : '—'}</b><span>HEAT-FLOW SLOPE</span><b>{ran ? '+0.42 mW' : '—'}</b></div>{ran && <div className="decision-stack"><p className="mini-label">DISPOSITION</p><button type="button" onClick={() => onFinish(true)}>Retain failure · inspect pan set</button><button type="button" className="secondary" onClick={() => onFinish(false)}>Digitally zero · release method</button></div>}</div></div>{feedback && <p className="feedback bad">{feedback}</p>}</ModalShell>;
}
...
function BlankControlModal({ setChecks, acquired, onAcquire, feedback, onFinish, onClose }: { checks: Record<string, boolean>; setChecks: React.Dispatch<React.SetStateAction<Record<string, boolean>>>; acquired: boolean; onAcquire: () => void; feedback: string; onFinish: (correct: boolean) => void; onClose: () => void }) {
  ...
  return <ModalShell title="Paired-pan blank verification" kicker="MEASUREMENT CONTROL · TGA-01 / PANSET-14" onClose={onClose} wide><div className="modal-grid scenario-bench-grid"><div><p className="modal-intro">The matched pans restore identity. The blank now tests whether the measurement channels are actually under control.</p><div className="evidence-brief">{items.map(([key, title]) => <article key={key}><i>•</i><div><b>{title}</b><small>{key === 'pair' ? 'Physical pair, method, and sample record agree.' : key === 'condition' ? 'Residue or damage would invalidate the comparison.' : 'Atmosphere and program context travel with the result.'}</small></div></article>)}</div><button className="modal-run" type="button" disabled={acquired} onClick={acquire}>{acquired ? 'COUPLED BLANK RETAINED' : 'ACQUIRE PAIRED-PAN BLANK'}</button>{!acquired && <button className="blank-release-shortcut" type="button" onClick={() => onFinish(false)}>Release from matched pan identity only</button>}</div><div className="instrument-console"><div className="panel-heading"><span>MASS (mg) + HEAT FLOW (mW) VS TIME</span><b>THM-208 · REV 4</b></div><TgaBlankTrace acquired={acquired} /><div className="result-box"><span>MASS OFFSET</span><b>{acquired ? '+0.03 mg' : '—'}</b><span>HEAT-FLOW SLOPE</span><b>{acquired ? '+0.04 mW' : '—'}</b></div>{acquired && <div className="blank-verdict"><span>METHOD CRITERIA</span><b>MASS + HEAT FLOW PASS</b><i>IN CONTROL</i></div>}{acquired && <div className="decision-stack"><button type="button" onClick={() => onFinish(true)}>Retain blank · release LOT-91-T</button></div>}</div></div>{feedback && <p className="feedback bad">{feedback}</p>}<p className="concept-note">Simulation boundary · pan conditioning, purge, temperature program, and acceptance limits remain method-specific.</p></ModalShell>;
}
