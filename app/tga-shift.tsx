...
type Modal = 'deck' | 'guide' | 'baseline' | 'pan' | 'evidence' | 'complete' | null;
...
export function TgaShift({ onSwitch }: { onSwitch: (id: ScenarioId) => void }) {
  ...
  const releaseBlank = () => { setPhase(3); setSelectedId('TGA-01'); reward({ uptime: scores.uptime + 6 }); appendLog('qc', 'Paired-pan blank completed inside mass and heat-flow baseline criteria; LOT-91-T released.', 16); };
  ...
  const actions = [() => open('baseline'), () => open('pan'), releaseBlank, advance, () => open('evidence'), () => open('complete')];
...
function TgaCompleteModal({ scores, logCount, exceptionCount, onDeck, onClose }: { scores: Scores; logCount: number; exceptionCount: number; onDeck: () => void; onClose: () => void }) {
  ...
  return <ModalShell title="Shift debrief" kicker="WO-2987 · COMPLETE" onClose={onClose}><div className="debrief-score"><span>SHIFT RATING</span><b>{total}</b><i>/ 100</i></div><p className="modal-intro">You retained a failed baseline, reconciled the governed pan pair, restored measurement control, and stopped a purge-coupled artifact from changing synthesis.</p><DebriefVisual scenario="tga" scores={scores} exceptionCount={exceptionCount} /><div className="debrief-grid"><span>Safety<b>{scores.safety}</b></span><span>Traceability<b>{scores.traceability}</b></span><span>Data integrity<b>{scores.integrity}</b></span><span>Uptime<b>{scores.uptime}</b></span></div><div className="lesson-card"><b>Technician leverage</b><p>Thermal curves only become scientific evidence when pan identity, atmosphere, baseline state, sample mass, and coupled channels remain linked.</p></div><p className="debrief-meta">{logCount} shift events captured · baseline and native traces retained</p><button className="modal-run" type="button" onClick={onDeck}>CHOOSE ANOTHER SHIFT</button></ModalShell>;
}
...
function LedgerDrawer({ log, onClose }: { log: LogItem[]; onClose: () => void }) { return <div className="drawer-backdrop" role="presentation" onClick={onClose}><aside className="ledger-drawer" role="dialog" aria-modal="true" aria-label="Event ledger" onClick={(event) => event.stopPropagation()}><header><div><p className="section-kicker">SHIFT RECORD</p><h2>Event ledger</h2></div><button type="button" onClick={onClose}>×</button></header><p className="drawer-intro">A chronological record of physical checks, baseline control, pan identity, native traces, and technician decisions.</p><ol>{[...log].reverse().map((item, index) => <li key={`${item.time}-${index}`}><time>{item.time}</time><i className={item.type}>{item.type}</i><p>{item.text}</p></li>)}</ol></aside></div>; }
