...
function FacilityBuildModal({ thermalBayLevel, stagingBayLevel, scheduled, insight, commissioningAvailable, queueMinutes, activeStationId, activeRunId, activeStatus, constraintKind, constraintLabel, constraintMetric, onCommission, onCommissionStorage, onViewAsset, onClose }: { thermalBayLevel: number; stagingBayLevel: number; scheduled: boolean; insight: number; commissioningAvailable: boolean; queueMinutes: number; activeStationId: string; activeRunId: string; activeStatus: string; constraintKind: string | null; constraintLabel: string; constraintMetric: string; onCommission: () => void; onCommissionStorage: () => void; onViewAsset: (stationId: string) => void; onClose: () => void }) {
  ...
  const handleStorageKey = (event: KeyboardEvent<SVGGElement>) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); onCommissionStorage(); } };
      ...
      <div className="facility-build-workspace">
        <div className={`facility-blueprint ${blueprintLayer}`}>
          ...
          <svg viewBox="0 0 760 430" role="group" aria-label={`Interactive top-down materials laboratory ${blueprintLayer === 'utilities' ? 'utility service overlay' : 'process flow layout'}`}>
            ...
            <g className={`facility-staging-bay ${storageQualified ? 'qualified' : 'open-socket'}`} role="button" tabIndex={0} aria-label={`${storageQualified ? 'Inspect' : 'Commission'} STG-02 vertical carousel`} onClick={onCommissionStorage} onKeyDown={handleStorageKey}><rect x="54" y="153" width="152" height="34" rx="2" /><text x="66" y="167">STG-02 · {storageQualified ? 'VERTICAL CAROUSEL' : 'EXPANSION SOCKET'}</text><text className="staging-state" x="194" y="180">{storageQualified ? 'QUALIFIED' : 'OPEN'}</text><path d="M66 177h55m7 0h12m6 0h12" /></g>
