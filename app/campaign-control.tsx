...
export function CampaignControlModal({ autoOpenInventory = false, onClose }: { autoOpenInventory?: boolean; onClose: () => void }) {
  ...
  return <div className="modal-backdrop campaign-backdrop" role="presentation">
    ...
    {facilityOpen && <FacilityBuildModal thermalBayLevel={run.thermalBayLevel} scheduled={Boolean(run.plannedThermalUpgrade)} insight={run.insight} commissioningAvailable={run.stage <= 3} queueMinutes={operations.queueMinutes} onCommission={() => { setFacilityOpen(false); setCommissionOpen(true); }} onViewAsset={viewAssetInLab} onClose={() => setFacilityOpen(false)} />}
...
function FacilityBuildModal({ thermalBayLevel, scheduled, insight, commissioningAvailable, queueMinutes, onCommission, onViewAsset, onClose }: { thermalBayLevel: number; scheduled: boolean; insight: number; commissioningAvailable: boolean; queueMinutes: number; onCommission: () => void; onViewAsset: (stationId: string) => void; onClose: () => void }) {
      ...
      <div className="facility-build-workspace">
        <div className="facility-blueprint">
          <svg viewBox="0 0 760 430" role="group" aria-label="Interactive top-down layout of materials laboratory and its process flow">
            ...
            <g className="build-asset" role="button" tabIndex={0} aria-label="Walk to PREP-01 powder preparation" onClick={() => onViewAsset('PREP-01')} onKeyDown={(event) => handleAssetKey(event, 'PREP-01')}><rect x="54" y="55" width="152" height="82" rx="3" /><text className="asset-id" x="66" y="73">PREP-01</text><text className="asset-name" x="66" y="88">POWDER PREPARATION</text><path d="M67 103h53v20H67zm65 0h60v20h-60z" /><circle cx="94" cy="113" r="7" /><text className="asset-state" x="146" y="116">ONLINE</text></g>
            <g className="build-asset" role="button" tabIndex={0} aria-label="Walk to ROBO-02 synthesis cell" onClick={() => onViewAsset('ROBO-02')} onKeyDown={(event) => handleAssetKey(event, 'ROBO-02')}><rect x="264" y="55" width="152" height="82" rx="3" /><text className="asset-id" x="276" y="73">ROBO-02</text><text className="asset-name" x="276" y="88">SYNTHESIS CELL</text><circle cx="317" cy="111" r="19" /><path d="M317 111l20-14 18 11 19-9M317 111l-9 13" /><circle cx="355" cy="108" r="4" /><text className="asset-state" x="362" y="126">ONLINE</text></g>
            <g className={`build-asset thermal ${qualified ? 'expanded' : scheduled ? 'scheduled' : 'open-socket'}`} role="button" tabIndex={0} aria-label="Walk to FURN-04 thermal bay" onClick={() => onViewAsset('FURN-04')} onKeyDown={(event) => handleAssetKey(event, 'FURN-04')}><rect x="474" y="45" width="152" height="102" rx="3" /><text className="asset-id" x="486" y="63">FURN-04</text><text className="asset-name" x="486" y="78">THERMAL BAY</text><g className="chamber-a"><rect x="488" y="91" width="58" height="42" /><text x="498" y="107">A</text><circle cx="528" cy="104" r="3" /><text className="chamber-state" x="498" y="123">ONLINE</text></g><g className="chamber-b"><rect x="558" y="91" width="54" height="42" /><text x="568" y="107">B</text><circle cx="596" cy="104" r="3" /><text className="chamber-state" x="568" y="123">{qualified ? 'ONLINE' : scheduled ? 'IQ/OQ' : 'OPEN'}</text></g></g>
            <g className="build-asset" role="button" tabIndex={0} aria-label="Walk to XRD-03 diffractometer" onClick={() => onViewAsset('XRD-03')} onKeyDown={(event) => handleAssetKey(event, 'XRD-03')}><rect x="54" y="278" width="152" height="82" rx="3" /><text className="asset-id" x="66" y="296">XRD-03</text><text className="asset-name" x="66" y="311">DIFFRACTION</text><path d="M69 341h122M81 341c19-2 20-21 32-21s11 16 24 16 12-9 21-9 10 7 20 7" /><text className="asset-state" x="151" y="351">ONLINE</text></g>
            <g className="build-asset" role="button" tabIndex={0} aria-label="Walk to SEM-01 BSE and EDS" onClick={() => onViewAsset('SEM-01')} onKeyDown={(event) => handleAssetKey(event, 'SEM-01')}><rect x="264" y="278" width="152" height="82" rx="3" /><text className="asset-id" x="276" y="296">SEM-01</text><text className="asset-name" x="276" y="311">BSE / EDS</text><circle cx="320" cy="334" r="15" /><path d="M320 313v42m-21-21h42" /><text className="asset-state" x="362" y="351">ONLINE</text></g>
