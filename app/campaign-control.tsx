...
export function CampaignControlModal({ autoOpenInventory = false, onClose }: { autoOpenInventory?: boolean; onClose: () => void }) {
  ...
  const activeLabStationStatus = run.stage === 0 ? 'NEXT' : run.stage === 7 ? 'REVIEW' : run.stage >= 8 ? 'DIAG' : 'ACTIVE';
  const viewInLab = () => {
...
function FacilityBuildModal({ thermalBayLevel, scheduled, insight, commissioningAvailable, queueMinutes, activeStationId, activeRunId, activeStatus, onCommission, onViewAsset, onClose }: { thermalBayLevel: number; scheduled: boolean; insight: number; commissioningAvailable: boolean; queueMinutes: number; activeStationId: string; activeRunId: string; activeStatus: string; onCommission: () => void; onViewAsset: (stationId: string) => void; onClose: () => void }) {
    ...
    <section className="facility-build" role="dialog" aria-modal="true" aria-label="Facility configuration">
      ...
      <div className="facility-build-workspace">
        <div className="facility-blueprint">
          <div className="facility-route-chip"><span>LIVE EXPERIMENT ROUTE</span><b>{activeRunId} → {activeStationId}</b><em>{activeStatus}</em></div>
          <svg viewBox="0 0 760 430" role="group" aria-label="Interactive top-down layout of materials laboratory and its process flow">
            ...
            <g className="route-branches"><path d="M130 137v78m210-78v78m210-68v68M130 215v63m210-63v63m210-63v63m136-45v45" /><path className="main-route" d="M58 215h586" /><path className="active-route-branch" d={activeBranch} /></g>
            ...
            <g className={`build-asset ${activeStationId === 'PREP-01' ? 'route-active' : ''}`} role="button" tabIndex={0} aria-label="Walk to PREP-01 powder preparation" onClick={() => onViewAsset('PREP-01')} onKeyDown={(event) => handleAssetKey(event, 'PREP-01')}><rect x="54" y="55" width="152" height="82" rx="3" /><text className="asset-id" x="66" y="73">PREP-01</text><text className="asset-name" x="66" y="88">POWDER PREPARATION</text><path d="M67 103h53v20H67zm65 0h60v20h-60z" /><circle cx="94" cy="113" r="7" /><text className="asset-state" x="146" y="116">{activeStationId === 'PREP-01' ? activeStatus : 'ONLINE'}</text></g>
            <g className={`build-asset ${activeStationId === 'ROBO-02' ? 'route-active' : ''}`} role="button" tabIndex={0} aria-label="Walk to ROBO-02 synthesis cell" onClick={() => onViewAsset('ROBO-02')} onKeyDown={(event) => handleAssetKey(event, 'ROBO-02')}><rect x="264" y="55" width="152" height="82" rx="3" /><text className="asset-id" x="276" y="73">ROBO-02</text><text className="asset-name" x="276" y="88">SYNTHESIS CELL</text><circle cx="317" cy="111" r="19" /><path d="M317 111l20-14 18 11 19-9M317 111l-9 13" /><circle cx="355" cy="108" r="4" /><text className="asset-state" x="362" y="126">{activeStationId === 'ROBO-02' ? activeStatus : 'ONLINE'}</text></g>
            <g className={`build-asset thermal ${qualified ? 'expanded' : scheduled ? 'scheduled' : 'open-socket'} ${activeStationId === 'FURN-04' ? 'route-active' : ''}`} role="button" tabIndex={0} aria-label="Walk to FURN-04 thermal bay" onClick={() => onViewAsset('FURN-04')} onKeyDown={(event) => handleAssetKey(event, 'FURN-04')}><rect x="474" y="45" width="152" height="102" rx="3" /><text className="asset-id" x="486" y="63">FURN-04</text><text className="asset-name" x="486" y="78">THERMAL BAY</text>{activeStationId === 'FURN-04' && <text className="asset-route-state" x="578" y="63">{activeStatus}</text>}<g className="chamber-a"><rect x="488" y="91" width="58" height="42" /><text x="498" y="107">A</text><circle cx="528" cy="104" r="3" /><text className="chamber-state" x="498" y="123">ONLINE</text></g><g className="chamber-b"><rect x="558" y="91" width="54" height="42" /><text x="568" y="107">B</text><circle cx="596" cy="104" r="3" /><text className="chamber-state" x="568" y="123">{qualified ? 'ONLINE' : scheduled ? 'IQ/OQ' : 'OPEN'}</text></g></g>
            <g className={`build-asset ${activeStationId === 'XRD-03' ? 'route-active' : ''}`} role="button" tabIndex={0} aria-label="Walk to XRD-03 diffractometer" onClick={() => onViewAsset('XRD-03')} onKeyDown={(event) => handleAssetKey(event, 'XRD-03')}><rect x="54" y="278" width="152" height="82" rx="3" /><text className="asset-id" x="66" y="296">XRD-03</text><text className="asset-name" x="66" y="311">DIFFRACTION</text><path d="M69 341h122M81 341c19-2 20-21 32-21s11 16 24 16 12-9 21-9 10 7 20 7" /><text className="asset-state" x="151" y="351">{activeStationId === 'XRD-03' ? activeStatus : 'ONLINE'}</text></g>
            <g className={`build-asset ${activeStationId === 'SEM-01' ? 'route-active' : ''}`} role="button" tabIndex={0} aria-label="Walk to SEM-01 BSE and EDS" onClick={() => onViewAsset('SEM-01')} onKeyDown={(event) => handleAssetKey(event, 'SEM-01')}><rect x="264" y="278" width="152" height="82" rx="3" /><text className="asset-id" x="276" y="296">SEM-01</text><text className="asset-name" x="276" y="311">BSE / EDS</text><circle cx="320" cy="334" r="15" /><path d="M320 313v42m-21-21h42" /><text className="asset-state" x="362" y="351">{activeStationId === 'SEM-01' ? activeStatus : 'ONLINE'}</text></g>
            ...
            <circle className="flow-token" cx={activeFlowX} cy="215" r="5" /><text className="aisle-label" x="57" y="390">MAIN PERSONNEL AISLE</text><path className="aisle" d="M56 377h665" /><text className="north" x="718" y="38">N ↑</text>
          </svg>
