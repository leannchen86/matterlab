...
function BenchModal({ scenarioId, ran, setRan, feedback, appendLog, onFinish, onClose }: { scenarioId: 'bet' | 'furnace'; ran: boolean; setRan: (value: boolean) => void; feedback: string; appendLog: (type: string, text: string, add?: number) => void; onFinish: (correct: boolean) => void; onClose: () => void }) {
  ...
  return <ModalShell title={isBet ? 'Check the repaired analyzer' : 'Secure the interrupted run'} kicker={isBet ? 'STEP 1 · MACHINE CHECK' : 'STEP 1 · INTERRUPTION CHECK'} onClose={onClose}><div className="modal-grid scenario-bench-grid"><div><p className="modal-intro">{isBet ? 'The repair is complete. Review what the lab check must prove, then run it.' : 'The furnace is safely stopped. Preserve the old run and secure the sample before doing anything else.'}</p><div className="evidence-brief">{items.map(([key, title, note]) => <article key={key}><i>•</i><div><b>{title}</b><small>{note}</small></div></article>)}</div><button className="modal-run" type="button" disabled={ran} onClick={run}>{ran ? 'EVIDENCE CAPTURED' : isBet ? 'RUN MACHINE CHECK · 12 MIN' : 'SAVE TRACE + HOLD SAMPLE · 5 MIN'}</button></div><div className={`instrument-console ${isBet ? 'vacuum-console' : 'thermal-console'}`}><div className="panel-heading"><span>{isBet ? 'VACUUM DECAY · RATE VS LIMIT' : 'TEMPERATURE VS TIME · °C / MIN'}</span><b>{isBet ? 'PORTS 1–4' : 'HT-44 · REV 7'}</b></div>{isBet ? <VacuumTrace ran={ran} /> : <ThermalTrace ran={ran} />}<div className="result-box"><span>{isBet ? 'REPAIR' : 'ALARM'}</span><b>{isBet ? 'COMPLETE' : 'I-204'}</b><span>{isBet ? 'LAB CHECK' : 'SAMPLE'}</span><b>{ran ? (isBet ? 'PASS' : 'SAFELY HELD') : '—'}</b></div>{ran && <div className="decision-stack"><p className="mini-label">WHAT SHOULD HAPPEN NEXT?</p><button type="button" onClick={() => onFinish(true)}>{isBet ? 'Accept the analyzer with this check' : 'Keep the trace and set the sample aside'}</button><button type="button" className="secondary" onClick={() => onFinish(false)}>{isBet ? 'Use the repair note without a lab check' : 'Resume from where heating stopped'}</button></div>}</div></div>{feedback && <p className="feedback bad">{feedback}</p>}</ModalShell>;
}
...
function ScenarioCompleteModal({ scenarioId, scores, logCount, exceptionCount, onDeck, onClose }: { scenarioId: 'bet' | 'furnace'; scores: Scores; logCount: number; exceptionCount: number; onDeck: () => void; onClose: () => void }) {
  ...
  return <ModalShell title="Mission debrief" kicker={`${isBet ? 'WO-2916' : 'WO-2954'} · COMPLETE`} onClose={onClose} wide={false}><div className="debrief-score"><span>RUN RATING</span><b>{total}</b><i>/ 100</i></div><p className="modal-intro">{isBet ? 'You checked the repaired analyzer, matched the sample record, and avoided changing the material from an uncertain reading.' : 'You preserved the interrupted heating history, found the physical sample, tested the empty equipment, and excluded the incomplete result.'}</p><DebriefVisual scenario={scenarioId} scores={scores} exceptionCount={exceptionCount} /><div className="debrief-grid"><span>Safety<b>{scores.safety}</b></span><span>Traceability<b>{scores.traceability}</b></span><span>Data integrity<b>{scores.integrity}</b></span><span>Uptime<b>{scores.uptime}</b></span></div><p className="score-explanation"><b>Score note:</b> Lower category scores reflect blocked choices and recovery time. {exceptionCount} blocked attempt{exceptionCount === 1 ? "" : "s"}.</p><div className="lesson-card"><b>Key lesson</b><p>{isBet ? 'A clean number is only trustworthy when the machine check, sample preparation, and reference result agree.' : 'Recovery means preserving the interrupted history, controlling the sample, and proving the equipment is ready.'}</p></div><p className="debrief-meta">{logCount} run events captured · all critical evidence retained</p><button className="modal-run" type="button" onClick={onDeck}>CHOOSE ANOTHER MISSION</button></ModalShell>;
}
