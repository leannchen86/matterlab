...
export function PlannerPanel({ scenario, phase }: { scenario: ScenarioId; phase: number }) {
  ...
  if (campaign.stage > 0) {
    ...
    const status = campaign.stage >= 9 ? 'DIAGNOSIS LINKED · LEARNING' : campaign.stage === 8 ? 'SEM / EDS FOLLOW-UP' : campaign.stage >= 7 ? spec.objectiveMet ? 'TARGET MET · LEARNING' : 'VALID NEGATIVE · LEARNING' : campaign.stage >= 6 ? 'MEASUREMENT GATE' : 'LAB EXECUTION';
    const request = campaign.stage >= 9 ? `Assimilate diagnosis · ${identity.runId}` : campaign.stage === 8 ? `Explain valid negative · ${identity.runId}` : campaign.stage >= 7 ? `Assimilate ${identity.runId} · ${spec.measured}%` : `Execute ${spec.id} · ${spec.formula}`;
    const gate = campaign.stage === 2 ? 'Robot cleanliness witness' : campaign.stage === 4 ? 'Capacity-one furnace queue' : campaign.stage === 6 ? 'NIST Si control required' : campaign.stage === 8 ? 'Four BSE fields + representative EDS map' : campaign.stage >= 9 ? `${spec.id === 'D-08' ? 'Ti-rich cores' : 'Ca-rich grains'} · hypothesis linked` : campaign.stage >= 7 ? `${spec.gap} objective gap · qualified result` : `${identity.runId} physical evidence`;
    const next = campaign.stage >= 9 ? 'Archive evidence + propose next candidate' : campaign.stage === 8 ? 'Acquire representative microscopy evidence' : campaign.stage >= 7 ? 'Archive result + propose next candidate' : campaign.stage >= 6 ? 'Acquire qualified diffraction pattern' : 'Advance governed material route';
...
function LedgerDrawer({ log, onClose }: { log: LogItem[]; onClose: () => void }) { return <div className="drawer-backdrop" role="presentation" onClick={onClose}><aside className="ledger-drawer" role="dialog" aria-modal="true" aria-label="Event ledger" onClick={(event) => event.stopPropagation()}><header><div><p className="section-kicker">RUN RECORD</p><h2>Event ledger</h2></div><button type="button" onClick={onClose}>×</button></header><p className="drawer-intro">A chronological record of operator checks, equipment state, material exceptions, results, and decisions.</p><ol>{[...log].reverse().map((item, index) => <li key={`${item.time}-${index}`}><time>{item.time}</time><i className={item.type}>{item.type}</i><p>{item.text}</p></li>)}</ol></aside></div>; }
