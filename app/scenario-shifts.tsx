...
const scenarios: Record<'bet' | 'furnace', Scenario> = {
  ...
  furnace: {
    ...
    tasks: [
      ...
      { title: 'Review the incomplete result', pending: 'Heating stopped at 61%', done: 'Incomplete result held' },
    ],
  },
};
...
export function AlternateShift({ scenarioId, onSwitch }: { scenarioId: 'bet' | 'furnace'; onSwitch: (id: ScenarioId) => void }) {
  ...
  const [log, setLog] = useState<LogItem[]>([
    { time: scenarioId === 'bet' ? '10:31' : '13:04', type: 'handoff', text: scenarioId === 'bet' ? 'Vendor service MX-233 marked complete; the lab must still check the analyzer.' : 'Alarm I-204 interrupted heating at 742 °C. The equipment was paused.' },
  ...
  const stations = useMemo(() => baseStations.map((station): Station => {
    if (scenarioId === 'bet' && station.id === 'BET-02') {
      ...
      if (phase < 3) return { ...station, state: 'READY', tone: 'ready', meta: phase === 1 ? 'Machine check complete · ports ready' : 'Sample record check', technicianView: ['Analysis ports: available', 'Leak/blank: passed', `Tube ADS-77-C: ${phase >= 2 ? 'matched' : 'hold'}`, 'N₂ supply: normal'] };
  ...
  const finishBench = (correct: boolean) => {
    ...
    appendLog('decision', scenarioId === 'bet' ? 'BET-02 accepted after service-record review and independent blank/leak evidence.' : 'BC-207 held; interrupted thermal trace retained and linked to an exception record.', 2);
  ...
  const finishSample = (correct: boolean) => {
    ...
    appendLog('lineage', scenarioId === 'bet' ? 'ADS-77-C held, source barcode reconciled, and correct degas record linked.' : 'BC-207 physical occupancy reconciled; load quarantined and robot state restored.', 2);
  ...
  const finishEvidence = (correct: boolean) => {
    if (!correct) { penalize('integrity', 17); setFeedback(scenarioId === 'bet' ? 'The suggestion blames the material before the low reference result is checked.' : 'The interrupted specimen did not complete the same heating cycle, so it should not be used for predictions.'); appendLog('exception', scenarioId === 'bet' ? 'A material change was suggested before the low reference was reviewed. The decision was blocked.' : 'The interrupted result was offered to the prediction model without a warning label. The export was blocked.', 3); return; }
    setPhase(5); reward({ integrity: scores.integrity + 12, traceability: scores.traceability + 5 }); appendLog('decision', scenarioId === 'bet' ? 'Low reference sent for recheck. Material changes remain paused until the measurement is trustworthy.' : 'Interrupted result labeled and excluded from predictions. A replacement run was queued.', 5); setModal('complete'); setFeedback('');
  };
...
function getActionState(id: 'bet' | 'furnace', phase: number, bench: () => void, sample: () => void, release: () => void, advance: () => void, evidence: () => void, complete: () => void) {
  const bet = [
    ...
    ['NEXT STEP', 'The analyzer is ready', 'The machine and sample records now agree.', '4 tubes', 'START TEST · 5 MIN', release, 'ready'],
    ['IN PROGRESS', 'The BET test is running', 'Complete the simulated acquisition to inspect the result.', '61%', 'COMPLETE TEST · 64 MIN', advance, 'run'],
  ...
  const furnace = [
    ...
    ['NEXT STEP', 'The equipment is working again', 'Now decide whether the interrupted result can be reused.', 'Checks passed', 'OPEN OLD RUN · 1 MIN', advance, 'ready'],
...
function BenchModal({ scenarioId, ran, setRan, feedback, appendLog, onFinish, onClose }: { scenarioId: 'bet' | 'furnace'; ran: boolean; setRan: (value: boolean) => void; feedback: string; appendLog: (type: string, text: string, add?: number) => void; onFinish: (correct: boolean) => void; onClose: () => void }) {
  ...
  const items = isBet ? [
    ...
    ['blank', 'Run a separate machine check', 'A blank and leak check—not vendor status alone—will prove the analyzer is ready.'],
  ] : [
  ...
  return <ModalShell title={isBet ? 'Check the repaired analyzer' : 'Secure the interrupted run'} kicker={isBet ? 'STEP 1 · MACHINE CHECK' : 'STEP 1 · INTERRUPTION CHECK'} onClose={onClose}><div className="modal-grid scenario-bench-grid"><div><p className="modal-intro">{isBet ? 'The repair note says complete. Decide whether to release samples now or spend twelve minutes proving the analyzer.' : 'The furnace stopped at 742 °C. Decide whether to resume quickly or spend five minutes preserving the interrupted state.'}</p><div className="evidence-brief">{items.map(([key, title, note]) => <article key={key}><i>•</i><div><b>{title}</b><small>{note}</small></div></article>)}</div><button className="modal-run" type="button" disabled={ran} onClick={run}>{ran ? 'EVIDENCE CAPTURED' : isBet ? 'RUN MACHINE CHECK · 12 MIN' : 'SAVE TRACE + HOLD SAMPLE · 5 MIN'}</button>{!ran && <button className="blank-release-shortcut" type="button" onClick={() => onFinish(false)}>{isBet ? 'Release from repair note · 0 min' : 'Resume heating at 742 °C · 0 min'}</button>}</div><div className={`instrument-console ${isBet ? 'vacuum-console' : 'thermal-console'}`}><div className="panel-heading"><span>{isBet ? 'VACUUM DECAY · RATE VS LIMIT' : 'TEMPERATURE VS TIME · °C / MIN'}</span><b>{isBet ? 'PORTS 1–4' : 'HT-44 · REV 7'}</b></div>{isBet ? <VacuumTrace ran={ran} /> : <ThermalTrace ran={ran} />}<div className="result-box"><span>{isBet ? 'REPAIR' : 'ALARM'}</span><b>{isBet ? 'COMPLETE' : 'I-204'}</b><span>{isBet ? 'LAB CHECK' : 'SAMPLE'}</span><b>{ran ? (isBet ? 'PASS' : 'SAFELY HELD') : '—'}</b></div>{ran && <div className="decision-stack"><p className="mini-label">CHOOSE THE NEXT STATE</p><button type="button" onClick={() => onFinish(true)}>{isBet ? 'Release analyzer with the passing check' : 'Keep the trace and set the sample aside'}</button><button type="button" className="secondary" onClick={() => onFinish(false)}>{isBet ? 'Release only from the repair note' : 'Resume the interrupted sample at 742 °C'}</button></div>}</div></div>{feedback && <p className="feedback bad">{feedback}</p>}</ModalShell>;
}
...
function SampleModal({ scenarioId, scanned, setScanned, feedback, appendLog, onFinish, onClose }: { scenarioId: 'bet' | 'furnace'; scanned: boolean; setScanned: (value: boolean) => void; feedback: string; appendLog: (type: string, text: string, add?: number) => void; onFinish: (correct: boolean) => void; onClose: () => void }) {
  ...
  return <ModalShell title={isBet ? 'Match the sample tube' : 'Find the sample carrier'} kicker="STEP 2 · PHYSICAL CHECK" onClose={onClose}><p className="modal-intro">{isBet ? 'The physical tube must match its own preparation record before the test.' : 'The computer says the move finished, but the physical carrier may still be in the furnace. Compare both.'}</p><div className="record-compare"><article><span>WHAT YOU SEE</span><EquipmentGlyph type={isBet ? 'bet' : 'furnace'} /><b>{isBet ? 'ADS-77-C' : 'FURNACE'}</b><p>{scanned ? (isBet ? 'Tube C · dry mass 0.412 g' : 'BC-207 present · 4 crucibles') : 'Waiting for your check'}</p></article><i>≠</i><article className={scanned ? 'mismatch-record' : ''}><span>COMPUTER RECORD</span><div className="record-code">{scanned ? (isBet ? 'ADS-77-B' : 'TRANSFER COMPLETE') : '••••••••'}</div><b>{isBet ? 'PREPARATION DG-09' : 'LAST COMMAND 13:03:41'}</b><p>{isBet ? '300 °C · finished 09:58' : 'Gripper empty · destination acknowledged'}</p></article></div>{!scanned ? <button className="modal-run" type="button" onClick={scan}>{isBet ? 'SCAN TUBE + CHECK RECORD' : 'CHECK THE CELL + COMPARE'}</button> : <div className="decision-stack horizontal record-actions"><button type="button" onClick={() => onFinish(true)}>{isBet ? 'Set tube aside and find its record' : 'Set the carrier aside and correct the record'}</button><button type="button" className="secondary" onClick={() => onFinish(false)}>{isBet ? 'Copy the neighboring tube’s record' : 'Trust the computer and clear the cell'}</button></div>}{feedback && <p className="feedback bad">{feedback}</p>}</ModalShell>;
}
...
function RecoveryVerificationModal({ setChecks, ran, onRun, feedback, onFinish, onClose }: { checks: Record<string, boolean>; setChecks: React.Dispatch<React.SetStateAction<Record<string, boolean>>>; ran: boolean; onRun: () => void; feedback: string; onFinish: (correct: boolean) => void; onClose: () => void }) {
  const items = [
    ...
    ['records', 'Alarm + robot record saved', 'The alarm, temperature trace, and recovery command stay linked.'],
  ];
  const run = () => { setChecks({ boundary: true, safety: true, records: true }); onRun(); };
  return <ModalShell title="Run an empty safety test" kicker="STEP 3 · RECOVERY TEST" onClose={onClose}><div className="modal-grid scenario-bench-grid"><div><p className="modal-intro">Before loading another sample, test the furnace and robot together while the cell is empty.</p><div className="evidence-brief">{items.map(([key, title, note]) => <article key={key}><i>•</i><div><b>{title}</b><small>{note}</small></div></article>)}</div><button className="modal-run" type="button" disabled={ran} onClick={run}>{ran ? 'EMPTY-CELL TEST SAVED' : 'RUN EMPTY SAFETY TEST · 16 MIN'}</button>{!ran && <button className="blank-release-shortcut" type="button" onClick={() => onFinish(false)}>Skip the test because the alarm is cleared</button>}</div><div className="instrument-console recovery-console"><div className="panel-heading"><span>FURNACE + ROBOT TEST</span><b>RECOVERY · I-204</b></div><RecoverySequence ran={ran} /><div className="recovery-state-grid"><span>OCCUPANCY<b>{ran ? 'EMPTY' : 'HOLD'}</b></span><span>ACCESS LOOP<b>{ran ? 'CLOSED' : '—'}</b></span><span>FURNACE<b>{ran ? 'READY' : 'HOLD'}</b></span><span>ROBOT<b>{ran ? 'HANDSHAKE' : 'PARKED'}</b></span></div>{ran && <div className="blank-verdict"><span>TEST RESULT</span><b>EMPTY CELL + STATE CHANGES PASS</b><i>READY</i></div>}{ran && <div className="decision-stack"><button type="button" onClick={() => onFinish(true)}>Save the test and return equipment to ready</button></div>}</div></div>{feedback && <p className="feedback bad">{feedback}</p>}<p className="concept-note">Simulation only · real recovery must follow site and equipment procedures.</p></ModalShell>;
}
...
function ScenarioEvidenceModal({ scenarioId, feedback, onFinish, onClose }: { scenarioId: 'bet' | 'furnace'; feedback: string; onFinish: (correct: boolean) => void; onClose: () => void }) {
  const isBet = scenarioId === 'bet';
  return <ModalShell title={isBet ? 'Review the low reading' : 'Review the interrupted result'} kicker="STEP 4 · RESULT CHECK" onClose={onClose}><div className="evidence-grid"><div className="trace-panel"><div className="panel-heading"><span>{isBet ? 'N₂ ADSORPTION CURVE' : 'HEATING HISTORY COMPARISON'}</span><b>{isBet ? '77 K · ALU-21' : 'PLANNED VS ACTUAL'}</b></div>{isBet ? <IsothermChart /> : <EligibilityChart />}</div><div className="report-panel"><div className="panel-heading"><span>{isBet ? 'BET RESULT' : 'RUN RECORD'}</span><b>{isBet ? 'method v3.1' : 'HT-44-207'}</b></div><div className="report-metric"><span>{isBet ? 'Surface area' : 'Recorded status'}</span><b>{isBet ? '168 m²/g' : 'complete*'}</b></div><div className="report-metric"><span>{isBet ? 'Expected range' : 'Heating completed'}</span><b>{isBet ? '173–191 m²/g' : '61%'}</b></div><div className="report-metric"><span>{isBet ? 'Curve fit' : 'Peak temperature'}</span><b>{isBet ? '0.9992' : '742 °C'}</b></div><div className="report-status warn-status">{isBet ? 'REFERENCE BELOW RANGE' : 'INTERRUPTED RUN'}</div><p>{isBet ? 'The curve is smooth, but the known reference is 5 m²/g below its range.' : 'The planned profile continues beyond the point where the actual trace stopped.'}</p></div></div><div className="ai-proposal"><div><span>AUTOMATED SUGGESTION · SIMULATED</span><h3>{isBet ? 'Lower the material’s heating temperature by 35 °C' : 'Add the interrupted run to the prediction model'}</h3><p>{isBet ? 'Generated from the 168 m²/g result.' : 'Generated from the recorded “complete” status.'}</p></div><b>WAITING</b></div><div className="decision-stack horizontal evidence-actions"><button type="button" onClick={() => onFinish(true)}>{isBet ? 'Recheck ALU-21 before changing the material' : 'Label interrupted and exclude from predictions'}</button><button type="button" className="secondary" onClick={() => onFinish(false)}>{isBet ? 'Lower temperature from the 168 m²/g result' : 'Use it because the record says complete'}</button></div>{feedback && <p className="feedback bad">{feedback}</p>}</ModalShell>;
}
