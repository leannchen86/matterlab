...
export function getCampaignStationView(station: Station, stage: number, selected: string, runNumber: number, thermalBayLevel = 1, missionId: CampaignMissionId = 'purity', resultElapsed = 0, resultMeasured = ''): Station {
  ...
  if (stage === 1) return { ...station, state: 'CAMPAIGN PREP', tone: 'run', meta: `${spec.id} formulation · ${identity.runId}`, technicianView: [`Formula: ${spec.formula}`,`Target mass: ${spec.targetMass}`, `Carrier: ${identity.carrier}`] };
  if (stage === 2 && operations.robotCondition === 'contamination') return { ...station, state: 'CLEANLINESS HOLD', tone: 'warn', meta: 'Gripper witness required', technicianView: ['Cell boundary: proven', 'Gripper: cleanliness fault', 'Witness coupon: due'] };
  if (stage === 2 && operations.robotCondition === 'grip-force') return { ...station, state: 'TOOLING CHECK', tone: 'warn', meta: 'Jaw-force witness required', technicianView: ['Cell boundary: proven', 'Jaw pads: inspect seating', 'Force witness: due'] };
  if (stage === 2) return { ...station, state: 'CELL READINESS', tone: 'run', meta: 'Tool identity + handshake', technicianView: ['Cell boundary: proven', 'Gripper: nominal', 'Carrier handshake: prove'] };
  if (stage === 3) return { ...station, state: 'DOSING', tone: 'run', meta: `${identity.carrier} · crucible dosing`, technicianView: [`Carrier: ${identity.carrier}`, 'Dose positions: 6', `${operations.robotCondition === 'nominal' ? 'Cell setup' : 'Gripper witness'}: passed`] };
  if (stage === 4) return { ...station, name: thermalBayLevel >= 2 ? 'Dual-chamber furnace' : station.name, state: thermalBayLevel >= 2 ? 'LANE B READINESS' : 'QUEUE HOLD', tone: 'warn', meta: `${operations.furnaceLane} · ${operations.queueMinutes} min`, technicianView: [`Assigned lane: ${operations.furnaceLane}`, `Chamber A: ${operations.activeFurnaceRun}`, `${thermalBayLevel >= 2 ? 'Readiness gate' : 'Estimated wait'}: ${operations.queueMinutes} min`] };
  if (stage === 5 && operations.furnaceCondition === 'thermocouple-drift') return { ...station, name: thermalBayLevel >= 2 ? 'Dual-chamber furnace' : station.name, state: 'TC OFFSET HOLD', tone: 'warn', meta: operations.furnaceResult, technicianView: [`Witness: ${operations.furnaceResult}`, 'Controller offset: verify', `Profile: ${spec.profile}`] };
  if (stage === 5 && operations.furnaceCondition === 'door-seal') return { ...station, name: thermalBayLevel >= 2 ? 'Dual-chamber furnace' : station.name, state: 'DOOR SEAL HOLD', tone: 'warn', meta: operations.furnaceResult, technicianView: [`Survey: ${operations.furnaceResult}`, 'Latch compression: verify', `Profile: ${spec.profile}`] };
  if (stage === 5) return { ...station, name: thermalBayLevel >= 2 ? 'Dual-chamber furnace' : station.name, state: 'START READINESS', tone: 'run', meta: `${operations.furnaceLane} · ${spec.temperature}`, technicianView: [`Profile: ${spec.profile}`, `Witness: ${operations.furnaceResult}`, `Carrier: ${identity.carrier}`] };
  if (stage === 6 && operations.referenceCondition === 'age-due') return { ...station, state: 'QC HOLD', tone: 'warn', meta: `Silicon QC · ${operations.referenceAgeHours} h`, technicianView: ['QC material: NIST SRM 640f', `Last QC check: ${operations.referenceAgeHours} h ago`, 'Sample testing: blocked'] };
