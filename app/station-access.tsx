...
export function StationAccess({ station, scenarioId = 'xrd', physicalChecks = [] }: { station: Station; scenarioId?: ScenarioId; physicalChecks?: string[] }) {
  ...
  return <>
    <button className="station-access-button" type="button" onClick={openStationAccess}><span>⌁</span><b>{campaignActive && physicalChecks.length < 3 ? 'INSPECT THIS MACHINE' : 'USE THIS MACHINE'}</b><i>{physicalChecks.length === 3 ? 'Inspection complete' : campaignActive ? `${physicalChecks.length} of 3 inspection points checked` : 'Open its controls and records'}</i><em>→</em></button>
...
function InstrumentMimic({ station }: { station: Station }) {
  ...
  return <svg viewBox="0 0 520 270" role="img" aria-label={`${stationId} live equipment mimic`} style={{ '--mimic': accent } as React.CSSProperties}>
    ...
    <g transform="translate(18 18)"><circle r="5" fill="var(--mimic)" /><text x="12" y="4" fill="#8fa3b3" fontSize="10" fontFamily="monospace">SIM / {stationId}</text></g>
  </svg>;
}
