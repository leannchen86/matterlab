...
The seven 3D stations and local walkthrough checks are schematic. Only the XRD bench and its requested follow-ups generate sample measurements; the walkthrough does not calibrate or operate physical hardware. The visible XRD beam path appears only during simulated acquisition. XRD activity does not imply a furnace run or a robot transfer. The tour restores the September 1 continuous 24-second camera/target spline and field-of-view easing through the full facility, with pause and exit controls. The tour uses the historical ENTER LAB → TOUR variant: distance 2.8–5.7, polar angle 1.05–1.55, and its original rendering without postprocessing. The older implementation inherited the current mode, so overview and walk launches looked substantially different. The aisle variant is now consistent regardless of entry mode; live OrbitControls remain unmounted during the animation.
