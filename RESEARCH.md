...
The seven 3D stations and local walkthrough checks are schematic. Only the XRD bench and its requested follow-ups generate sample measurements; the walkthrough does not calibrate or operate physical hardware. The visible XRD beam path appears only during simulated acquisition. XRD activity does not imply a furnace run or a robot transfer. The tour uses stationary instrument views with transitions, pause and previous/next controls; it isolates the featured instrument as the focus view does.
